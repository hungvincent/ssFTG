...
$(document).ready(function() {
	$(document).on('click', 'tr.dataRow', function(){
		...
		if($(this).closest("tr").children('td').eq(0).text() != ""){
			...
			$("#princeStarId").val($(this).closest("tr").children('td').eq(0).text());
			$.ajax({ 
				url:'queryTypeByPrinceStarId',
				data:{ 
					id: $("#princeStarId").val()
				}, 
				type:'post', 
				...
				success:function(data) { 
					if(data.status == "success"){ 
						setPrinceStarTypeTbl(data.list);
					}
				}, 
				error: function() { 
					alert("queryTypeByPrinceStarId error...."); 
				} 
			});
		}
	});
...
function setPrinceStarTbl(json){
	$('#psTbl > tbody > tr').each(function(index, tr) {
    	if(index > 0){
    		$(this).remove();
    	}
   	});
	
	$('#pstTbl > tbody > tr').each(function(index, tr) {
    	if(index > 0){
    		$(this).remove();
    	}
   	});
	...
	for (var key in json) {
		$('#psTbl').append(
	        $('<tr class="dataRow"></tr>').append(
	            ...
	            $('<td style="text-align:center; vertical-align: middle;"></td>').html("<button type='button' class='btn btn-info btn-xs' onclick='updPrinceStar(this);'>修改</button> ")
	        )
	    );
		//代入排序第一筆的種類
		if(cnt == 0){
			$('#princeStarId').val(json[key].id);
			$("#psTbl tr").eq(1).css('color', '#000000');
			$("#psTbl tr").eq(1).css('background-color', 'yellow');
			
			if(json[key].princeStarType != null){
				setPrinceStarTypeTbl(json[key].princeStarType);
			}
		}
...
function setPrinceStarTypeTbl(json){
	$('#pstTbl > tbody > tr').each(function(index, tr) {
    	if(index > 0){
    		$(this).remove();
    	}
	...
	for (var key in json) {
		$('#pstTbl').append(
	        $('<tr></tr>').append(
	            ...
	            $('<td style="text-align:center; vertical-align: middle;"></td>').html("<button type='button' class='btn btn-info btn-xs' onclick='updPrinceStarType(this);'>修改</button> ")
	        )
	    );
	}
}
...
function addPrinceStarRow(){
	...
	$('#psTbl').append(
        $('<tr class="dataRow"></tr>').append(
            ...
            $('<td style="text-align:center; vertical-align: middle;"></td>').html("<button type='button' class='btn btn-info btn-xs' onclick='savePrinceStar(this);'>儲存</button>")
        )
    );
}

function addPrinceStarTypeRow(){
	if($("#princeStarId").val() == ""){
		alert("請選擇場次");
		return;
	}
	...
	$('#pstTbl').append(
        $('<tr></tr>').append(
            ...
            $('<td style="text-align:center; vertical-align: middle;"></td>').html("<input type='text' style='color:black; width:50px;' onblur='checkNum(this);' autocomplete='off' />"),
            $('<td style="text-align:center; vertical-align: middle;"></td>').html("<button type='button' class='btn btn-info btn-xs' onclick='savePrinceStarType(this);'>儲存</button>")
        )
    );
}

//修改
function updPrinceStar(obj){
    ...
    $td.eq(6).html("<button type='button' class='btn btn-info btn-xs' onclick='savePrinceStar(this);'>儲存</button>");
}

function updPrinceStarType(obj){
	var $td = $(obj).closest("tr").children();
    ...
    $td.eq(5).html("<button type='button' class='btn btn-info btn-xs' onclick='savePrinceStarType(this);'>儲存</button>")
}

//儲存
function savePrinceStar(obj){
	...
	$.ajax({ 
		...
		success:function(data) { 
			if(data.status == "success"){
				$("#princeStarId").val(data.princeStarId);
				
				if(data.types == "Y" && confirm(data.message)){
					$.ajax({ 
						url:'importPrinceStarType',
						data:{
							id: $("#princeStarId").val()
						},
						type:'post', 
						...
						success:function(data) { 
							if(data.status == "success"){
								alert(data.message);
								setPrinceStarTbl(data.list);
							}
						}, 
						error: function() { 
							alert("importPrinceStarType error...."); 
						} 
					});
				}else{
					$.ajax({ 
		...
		error: function() { 
			alert("editPrinceStar error...."); 
			$(obj).attr("disabled", false);
		} 
	});
}

function savePrinceStarType(obj){
	$(obj).attr("disabled", true);
	var $td = $(obj).closest("tr").children();
	...
	var o = new Object();
	o.princeStarId = $("#princeStarId").val();
	o.id = $td.eq(0).text();
	...
	$.ajax({ 
		...
		success:function(data) { 
			if(data.status == "success"){ 
				...
				setPrinceStarTypeTbl(data.types);
			}
			alert(data.message);
			$(obj).attr("disabled", false);
		}, 
		error: function() { 
			alert("editPrinceStarType error...."); 
			$(obj).attr("disabled", false);
		} 
	});
}
...
<div id="sec8" style="text-align:left;">
	<input type="hidden" style="color:black;" id="princeStarId">
		...
		<tr>
			<td style="width:58%;">
				<div style="height:600px; overflow-y:scroll;">
					<table id="psTbl" class="table table-bordered">
					<tr style="line-height:35px; text-align:center; background-color:#F7F7F7; color:#333; font-weight:bold;">
						...
						<th style="text-align:center;">
							<button type="button" class="btn btn-info btn-xs" onclick="addPrinceStarRow();">新增</button>
						</th>
					</tr>
					</table>
				</div>
			...
			<td style="width:40%;">
				<div style="height:600px; overflow-y:scroll;">
					<table id="pstTbl" class="table table-bordered">
					<tr style="line-height:35px; text-align:center; background-color:#F7F7F7; color:#333; font-weight:bold;">
						...
						<th style="text-align:center;">
							<button type="button" class="btn btn-info btn-xs" onclick="addPrinceStarTypeRow();">新增</button>
						</th>
					</tr>
					</table>
				</div>
