	...
	for (var key in json) {
		$('#conTbl').append(
	        $('<tr></tr>').append(
	            ...
	            $('<td style="text-align:center; vertical-align: middle;"></td>').html("<button type='button' class='btn btn-info btn-xs' onclick='updCon(this);'>修改</button> ")
	        )
	    );
	}
}
...
function addConRow(){
	$('#conTbl').append(
        $('<tr></tr>').append(
            ...
            $('<td style="text-align:center; vertical-align: middle;"></td>').html("<button type='button' class='btn btn-info btn-xs' onclick='saveCon(this);'>儲存</button>")
        )
    );
}
...
function updCon(obj){
    ...
    $td.eq(6).html("<button type='button' class='btn btn-info btn-xs' onclick='saveCon(this);'>儲存</button>");
}

//儲存
function saveCon(obj){
		...
		<table id="conTbl" class="table table-bordered">
			<tr style="line-height:35px; text-align:center; background-color:#F7F7F7; color:#333; font-weight:bold;">
				...
				<th style="text-align:center;">
					<button type="button" id="add" class="btn btn-info btn-xs" onclick="addConRow();">新增</button>
				</th>
			</tr>
		</table>
	</div>
