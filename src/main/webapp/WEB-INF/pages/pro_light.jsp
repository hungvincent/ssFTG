	...
	for (var key in json) {
		$('#liTbl').append(
	        $('<tr class="dataRow"></tr>').append(
	            ...
	            $('<td style="text-align:center; vertical-align: middle;"></td>').html("<button type='button' class='btn btn-info btn-xs' onclick='updLight(this);'>修改</button> ")
	        )
	    );
		//代入排序第一筆的種類
		if(cnt == 0){
	...
	for (var key in json) {
		$('#litTbl').append(
	        $('<tr></tr>').append(
	            ...
	            $('<td style="text-align:center; vertical-align: middle;"></td>').html("<button type='button' class='btn btn-info btn-xs' onclick='updLightType(this);'>修改</button> ")
	        )
	    );
	}
}
...
function addLightRow(){
	...
	$('#liTbl').append(
        $('<tr class="dataRow"></tr>').append(
            ...
            $('<td style="text-align:center; vertical-align: middle;"></td>').html("<button type='button' class='btn btn-info btn-xs' onclick='saveLight(this);'>儲存</button>")
        )
    );
}
...
function addLightTypeRow(){
	...
	$('#litTbl').append(
        $('<tr></tr>').append(
            ...
            $('<td style="text-align:center; vertical-align: middle;"></td>').html("<input type='text' style='color:black; width:70px;' onblur='checkNum(this);' autocomplete='off' />"),
            $('<td style="text-align:center; vertical-align: middle;"></td>').html("<button type='button' class='btn btn-info btn-xs' onclick='saveLightType(this);'>儲存</button>")
        )
    );
}
...
function updLight(obj){
    ...
    $td.eq(6).html("<button type='button' class='btn btn-info btn-xs' onclick='saveLight(this);'>儲存</button>");
}
...
function updLightType(obj){
    ...
    $td.eq(5).html("<button type='button' class='btn btn-info btn-xs' onclick='saveLightType(this);'>儲存</button>")
}
				...
				<div style="height:600px; overflow-y:scroll;">
					...
					<tr style="line-height:35px; text-align:center; background-color:#F7F7F7; color:#333; font-weight:bold;">
						...
						<th style="text-align:center;">
							<button type="button" class="btn btn-info btn-xs" onclick="addLightRow();">新增</button>
						</th>
					</tr>
					</table>
				</div>
			...
			<td style="width:40%;">
				<div style="height:600px; overflow-y:scroll;">
					...
					<tr style="line-height:35px; text-align:center; background-color:#F7F7F7; color:#333; font-weight:bold;">
						...
						<th style="text-align:center;">
							<button type="button" class="btn btn-info btn-xs" onclick="addLightTypeRow();">新增</button>
						</th>
					</tr>
					</table>
				</div>
